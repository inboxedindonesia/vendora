...
$(document).on("submit", "form", function (e) {
  var element = this;
  var button =
    e.originalEvent !== undefined && e.originalEvent.submitter !== undefined
      ? e.originalEvent.submitter
      : "";

  if (
    $(element).attr("data-oc-toggle") == "ajax" ||
    $(button).attr("data-oc-toggle") == "ajax"
  ) {
    e.preventDefault();
    ...
    var method =
      $(button).attr("formmethod") || $(form).attr("method") || "post";
    var enctype =
      $(button).attr("formenctype") ||
      $(form).attr("enctype") ||
        ...
        $.ajax({
          ...
          success: function (json) {
            ...
            if (json["code"]) {
              ...
              $(element)
                .parent()
                .find("[data-oc-toggle='download'], [data-oc-toggle='clear']")
                .prop("disabled", false);
            }
          },
          error: function (xhr, ajaxOptions, thrownError) {
...
$(document).on("click", "[data-oc-toggle='download']", function (e) {
  ...
  if (value != "") {
    location =
      "index.php?route=tool/upload.download&user_token=" +
...
$(document).on("click", "[data-oc-toggle='image']", function (e) {
  ...
  $.ajax({
    url:
      ...
      getURLVar("user_token") +
      "&target=" +
      encodeURIComponent($(element).attr("data-oc-target")) +
      "&thumb=" +
      encodeURIComponent($(element).attr("data-oc-thumb")),
    dataType: "html",
    beforeSend: function () {
      $(element).button("loading");
    },
  ...
  $("#nav-language .dropdown-item").on("click", function (e) {
    ...
    $.ajax({
      ...
      data:
        ...
        $(element).attr("href") +
        "&redirect=" +
        encodeURIComponent($("#input-redirect").val()),
      dataType: "json",
      success: function (json) {
